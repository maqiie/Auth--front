  ...
  const handleResetPassword = async (event) => {
    ...
    try {
      const response = await axios.put("/auth/password", {
        ...
        reset_password_token: resetToken, // Important: include token from query param
      });
    ...
    <div className="container mx-auto my-10 p-8 bg-white rounded-xl shadow-xl max-w-md">
      ...
      <form onSubmit={handleResetPassword}>
        ...
        <button
          type="submit"
          disabled={loading}
          className={`w-full p-4 text-lg font-semibold text-white rounded-lg transition duration-300 ease-in-out ${loading ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500'}`}
        >
